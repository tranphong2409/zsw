...
sap.ui.controller("sap.ui.project.e2etm.controller.CardReloadReport", {
	...
	onSearch:function(evt){
		...
		if(sCheck == true){
			...
			if(sFromDate != "" && sToDate == ""){
				var oDateFilter = new sap.ui.model.Filter({ path: "RECDT", operator: "EQ", value1: sFromDate });
				aFilters.push(oDateFilter);
			}

			if(sFromDate == "" && sToDate != ""){
				var oDateFilter = new sap.ui.model.Filter({ path: "RECDT", operator: "EQ", value1: sToDate});
				aFilters.push(oDateFilter);
			}

			if(sFromDate != "" && sToDate != ""){
				var oDateFilter = new sap.ui.model.Filter({ path: "RECDT", operator: "BT", value1: sFromDate, value2: sToDate });
				aFilters.push(oDateFilter);
			}
		}
...
//	},
	onExportExcel : function(oEvent) {
		...
		oExport = new sap.ui.core.util.Export({
			...
			rows : {
				path : "/ZE2E_CARD_RELOAD_REPORTSet"
			},
			columns : columns
		});

	oExport.saveFile("Card reload Report").always(function() {
		this.destroy();
	});
	},
